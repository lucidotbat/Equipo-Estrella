...
async function sumarGastosMesActual() {
    ...
    if (gastos.length != 0) {
        ...
        gastos.forEach(gasto => {
            ...
            if (mesGasto === mesActual && anioGasto === anioActual && gastos[i].diaDePago === sessionStorage.getItem('user')) {
...
async function calcularResultado() {
    ...
    if (ingresos.length != 0) {
        ingresos.forEach(ingreso => {
            if(ingreso.diaDePago == sessionStorage.getItem('user')) sumaIngresos += parseFloat(ingreso.ingresoMensual);        });
    }
    ...
    if (gastos.length != 0) {
        ...
        gastos.forEach(gasto => {
            ...
            if (mesGasto === mesActual && anioGasto === anioActual && gasto.User == sessionStorage.getItem('user')) {
                sumaGastos += parseFloat(gasto.Cantidad);
            }
        });
    }
