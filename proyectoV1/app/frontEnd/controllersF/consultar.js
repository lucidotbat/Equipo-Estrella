...
document.addEventListener("DOMContentLoaded", function() {
    ...
    btnReciente.addEventListener('click', function() {
     ...
     btnSearch.addEventListener('click', function() {
        for (let i = 0; i < gastos.length; i++){
            console.log(gastos[i].Empresa);
            listFilter1 = [];
            let toFilter = document.getElementById("inputFilter").value.toLowerCase();

            if (toFilter !== '') { // Verificar si toFilter no está vacío
                for (let i = 0; i < gastos.length; i++){
                    let empresa = gastos[i].Empresa.toLowerCase();
                    if (empresa.includes(toFilter)){

                        console.log("se encontró uno", toFilter);
                        listFilter1.push(gastos[i]);
                    }
                }
            }
